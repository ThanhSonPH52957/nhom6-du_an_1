...
class Phong
{
    ...
    public function getPhongDat($tai_khoan_id, $search_trang_thai = '')
    {
        ...
        if ($search_trang_thai) {
            $sql .= " AND trang_thai_dat_phongs.ten_trang_thai LIKE :search_trang_thai";
        }

        $sql .= " ORDER BY dat_phongs.ngay_dat DESC";

        $stmt = $this->conn->prepare($sql);
        $params = [':tai_khoan_id' => $tai_khoan_id];

        if ($search_trang_thai) {
            $params[':search_trang_thai'] = '%' . $search_trang_thai . '%';
        }
    ...
    public function phongDat($tai_khoan_id)
    {
        $sql = "SELECT dat_phongs.*, phongs.ten_phong, trang_thai_dat_phongs.ten_trang_thai
            ...
            INNER JOIN trang_thai_dat_phongs ON dat_phongs.trang_thai_id = trang_thai_dat_phongs.id
            WHERE dat_phongs.tai_khoan_id = :tai_khoan_id
            ORDER BY dat_phongs.ngay_dat DESC";
